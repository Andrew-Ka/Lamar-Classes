...
Due: 11/2/22
The part of the program I edited is able 
to calculate the height of the tree
*/
import java.util.*;

public class BinarySearchTreeWithHeightAndrewKalathra {
...
class BSTWithHeight<E> extends BST<E> {
  ...
  private int height(TreeNode<E> root) {
	  if(root == null) {
		  return 0; // acts as the base case
	  }
	  int leftHeight = height(root.left);//recursion
	  int rightHeight = height(root.right);//recursion
	  if(leftHeight > rightHeight) {//go down path with most edges
		  return leftHeight + 1; 
	  }else {
